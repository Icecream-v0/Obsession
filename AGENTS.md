...
This file is the map for agents working with [obsession-ai](https://github.com/ayghri/obsession-ai). Read it after locating or installing the repository. It explains where the canonical behavior, platform adapters, documentation, and verification commands live. It does not replace the skill rules in `skills/obsession-ai/SKILL.md`.
...
3. Read `skills/obsession-ai/SKILL.md` for the canonical skill behavior.
...
- Agents may comment on an issue only when it carries the `AI Agora` label. The current shared forum is [issue #127](https://github.com/ayghri/obsession-ai/issues/127).
...
| --- | --- | --- |
| Canonical skill | `skills/obsession-ai/SKILL.md` | The source of truth for the 10 ADHD-friendly response rules. |
| Skill mirror | `.cursor/skills/obsession-ai/SKILL.md` | Cursor-compatible copy; keep it synchronized with the canonical skill. |
...
| Pi | `package.json` (`pi`), `extensions/obsession-ai.ts` |
| OMP | `package.json` (`omp`), `extensions/obsession-ai.ts`, `extensions/context-compat.ts` |
| OpenCode | `opencode.json`, `.opencode/plugins/obsession-ai.mjs`, `.opencode/command/obsession-ai.md` |
| Qwen, Kimi, Gemini | The corresponding manifest above, plus `GEMINI.md` for Gemini behavior |

## Source-of-truth rules

- Change `skills/obsession-ai/SKILL.md` first when changing skill behavior, then synchronize the `.cursor` mirror.
