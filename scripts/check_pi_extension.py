...
def validate_package_manifest() -> None:
    ...
    assert package.get("pi") == {
        "extensions": ["./extensions/obsession-ai.ts"],
        "skills": ["./skills"],
    }
...
def status_texts(events: list[dict[str, Any]]) -> list[str | None]:
    return [
        ...
        and event.get("statusKey") == "obsession-ai"
    ]
...
def latest_enabled(entries_response: dict[str, Any]) -> bool:
    states = [
        ...
        and entry.get("customType") == "obsession-ai-state"
    ]
    if not states or not isinstance(states[-1], bool):
        raise AssertionError("No persisted obsession-ai state found")
    return states[-1]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test the obsession-ai extension without a model request."
    )
    parser.add_argument(
        "--runtime",
        choices=("pi", "omp"),
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"obsession-ai-{args.runtime}-"
    ) as agent_dir:
...
"""
        ...
        if args.runtime == "omp":
            try:
                ...
                assert "obsession-ai" in command_names
                assert "skill:obsession-ai" in command_names
                assert any(
                    "ADHD ON" in (text or "")
        ...
        try:
            ...
            assert message_count(entries, "obsession-ai-rules") == 1
            assert message_count(entries, "obsession-ai-disabled") == 0
            ...
            toggled_off, toggle_off_events = client.request(
                "toggle-off",
                {"type": "prompt", "message": "/obsession-ai"},
            )
            ...
            assert message_count(entries, "obsession-ai-disabled") == 1
            ...
            assert message_count(entries, "obsession-ai-rules") == 2

            explicit_off, _ = client.request(
                "explicit-off",
                {"type": "prompt", "message": "/obsession-ai off"},
            )
            assert explicit_off["success"] is True

            skill, skill_events = client.request(
                "skill-alias",
                {"type": "prompt", "message": "/skill:obsession-ai"},
            )
            ...
            assert latest_enabled(entries) is True
            assert message_count(entries, "obsession-ai-rules") == 3
        ...
        if args.runtime == "pi":
            Path(agent_dir, ".obsession-ai-always").touch()
            always_on = RpcClient(
                executable,
                env,
                "--no-session",
