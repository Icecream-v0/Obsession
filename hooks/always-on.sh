#!/usr/bin/env sh
# SessionStart hook: injects the full obsession-ai ruleset when the user has
# opted in by creating $CLAUDE_CONFIG_DIR/.obsession-ai-always (default ~/.claude).
...
flag_path="$claude_dir/.obsession-ai-always"
...
skill_path="$script_dir/../skills/obsession-ai/SKILL.md"
[ -f "$skill_path" ] || exit 0
...
printf 'ADHD MODE ACTIVE (always-on). The ruleset below applies to every response. "stop obsession mode" turns it off for this session; delete %s to turn always-on off for good.\n\n%s\n' \
  "$flag_path" "$body"
