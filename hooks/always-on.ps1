...
# obsession-ai ruleset when the user has opted in by creating
# $CLAUDE_CONFIG_DIR/.obsession-ai-always (default ~/.claude).
...
try {
  ...
  $flagPath = Join-Path $claudeDir ".obsession-ai-always"
  ...
  $skillPath = Join-Path $scriptDir "../skills/obsession-ai/SKILL.md"
  ...
  $banner = 'ADHD MODE ACTIVE (always-on). The ruleset below applies to every response. ' +
    '"stop obsession mode" turns it off for this session; delete '
